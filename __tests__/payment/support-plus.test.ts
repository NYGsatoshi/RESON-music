...
describe('Support+ monthly billing', () => {
  ...
  test('uses the immutable billing batch id as the Stripe idempotency key', async () => {
    const rpc = jest
      .fn()
    ...
    expect(result).toEqual({ prepared: 1, invoiceItemsCreated: 1 })
  })
  ...
  test('does not call Stripe when there are no pending Support+ batches', async () => {
    ...
    expect(result).toEqual({ prepared: 0, invoiceItemsCreated: 0 })
  })
